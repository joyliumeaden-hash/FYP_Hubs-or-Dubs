...
def run_single_optimization(solarCFs, windCFs, requiredMWh):
    ...
    #150 is just a value 
    solarMWinit = 300*np.ones(numSolar)   #initial solar generation guess of 150MW
    windMWinit = 300*np.ones(numWind)   #initial wind generation
    batteryMWinit = 300                  #battery discharge rate? 
...
#numLocs = len(lat_val)
numLocs = 1  #manually input 10 as not all data has been downloaded
#state = "VIC data/"
...
if(True):
    ...
    for loc in range(numLocs):
        ...
        model_result.append([text_val[locs],solarMWs[0],windMWs[0],batteryMW,annualized_cost,lcoe,plant_cost,penalty_cost])
        ...
        df.to_csv(directory_main + "single_test/" + "Model Result Nelder.csv", index=False) 
        ...
        for i in data_years:      
            filename_solar = directory_main + str(state_val[locs]) + " data/" + str(i) + "_pv_" + str(lat_val[locs]) + "_" + str(lon_val[locs]) + ".csv"
            filename_wind = directory_main + str(state_val[locs]) +  " data/" + str(i) + "_wind_" + str(lat_val[locs]) + "_" + str(lon_val[locs]) + ".csv"
        ...
        for i in data_years:   
            ...
            future_plant_cost = PlantAnnualizedCostFunction(solarMWs, windMWs, batteryMW) + np.sum(deficit_future1) * 20000
            future_LCOE = future_plant_cost/np.sum(requiredMWh)
            #print("plant cost in" + str(i) + ":  " + str(future_plant_cost))
            #print("                               lcoe:  " + str(future_LCOE))
            """
        ...
        results_df.to_csv(directory_main + "single_test/" "multiyear.csv", index=False)
